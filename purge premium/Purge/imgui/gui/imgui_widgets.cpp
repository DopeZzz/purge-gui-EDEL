...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? GetColorWithAlpha(main_color, 1.f) : GetColorWithAlpha(main_color, 0.f), g.IO.DeltaTime * 12.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, background_color, style.FrameRounding);
...
bool ImGui::Tab(const char* icon, int* tab, int number)
{
    ...
    const float DRAW_H = 30.f;
    const float HIT_PAD_TOP = 10.f;
    const float HIT_PAD_BOT = 4.f;

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect draw_bb(pos, pos + ImVec2(80, DRAW_H));
    const ImRect hit_bb(draw_bb.Min - ImVec2(0, HIT_PAD_TOP), draw_bb.Max + ImVec2(0, HIT_PAD_BOT));

    ItemSize(draw_bb.GetSize());
    if (!ItemAdd(hit_bb, id))
        return false;
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(hit_bb, id, &hovered, &held);
    if (pressed)
    {
        size_change = true;
        *tab = number;
    ...
    window->DrawList->AddRectFilled(draw_bb.Min - ImVec2(2, 0), draw_bb.Min + ImVec2(it_anim->second.line_offset, DRAW_H + HIT_PAD_BOT), GetColorU32(main_color), 90.f, ImDrawFlags_RoundCornersRight);

    PushFont(tab_font);
    window->DrawList->AddText(draw_bb.Min + ImVec2(40 - CalcTextSize(icon).x * 0.5f, DRAW_H * 0.5f - CalcTextSize(icon).y * 0.5f), GetColorU32(it_anim->second.text_color), icon);
    PopFont();
...
bool ImGui::SimpleCheckbox(const char* label, bool* v, const char* hint)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SoloKeybinBox(const char* label, const char* hint)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderDouble(const char* label, double* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags, hint);
}
